...
const agentsField = ['agentNo', 'gender', 'name', 'followId', 'level', 'mobile', 'latitude', 'longitude', 'nickname', 'category',
  'origin', 'type', 'lastFollowUpTime', 'nextFollowUpTime', 'obtainTime', 'followUpStatus',
...
const cluesField = ['clueNo', 'followId', 'name', 'record', 'mobile', 'intentionType', 'gender', 'responsibleId',
  ...
  'failTime', 'lastFollowUpTime', 'nextFollowUpTime', 'createTime', 'estimatedPurchaseTime', 'remark', 'belongStoreId', 'hasFollowUpRecord', 'hasFollow'];
const selectField = {
  customers: {
    all: customersField,
 ...
 * @param module         模块
 */
export const responsibleCareOf = ({id, responsibleId, module}) => {
  if (module === 'order') {
    return request({
  ...
  return request({
    url: `${CRM_IP}/wap/${type}/${id}/${responsibleId}/careOf`,
    method: 'put'
  });
};
 ...
 * @param id
 */
export const getEnquiryPricesDetail = (id) => {
  return request({
    url: `${CRM_IP}/wap/enquiryPrices/${id}/clue/get`,
    method: 'get',
    loading: false
  });
};
...
export const getGroundStore = () => {
  return request({
    url: `${SYS_IP}/wap/groundStore/all`,
    loading: false
  });
};
