...
import {isFastExecute} from '../utils/commonUtil';
...
export const ddApi = {
  ...
  prompt: (obj) => {
    return dd.ready(() => {
      dd.device.notification.prompt(obj);
    });
  },
  // 震动
  vibrate: (obj) => {
  ...
  // 获取省市区
  getAdress: ({title = '省市区', selectAdress = [8, 7, 4], onSuccess}) => {
    console.log(vue);
    let cascadePicker = vue.$createCascadePicker({
      title,
      data: vue.$store.state.user.addressList,
