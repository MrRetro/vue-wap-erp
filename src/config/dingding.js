 ...
 * @param store
 */
export const authConfig = (store) => new Promise(async (resolve, reject) => {
  try {
    const res = await getSignatures();
    store.dispatch('user/changeConfig', res);
    dd.config({
    ...
    dd.error((err) => {
      ...
      // RouteUtil.error({message: `钉钉配置错误${JSON.stringify(err)}`});
  ...
  resolve(store.dispatch('user/getDingDingUserInfo'));
