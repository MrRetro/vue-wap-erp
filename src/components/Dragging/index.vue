  ...
  <div class="dragging-box" :class="isShow && isShowLoc ?'show':'hide'">
    <div class="drag-content" :class="isShow && isShowLoc ?'show':'hide'">
      ...
      <div class="menu-list" v-on:scroll.passive="()=>{}">
        <div v-for="item in menus" :key="item.id" class="menu-li">
          <p class="title">{{item.name}}</p>
          <ul class="item-box">
            <li v-for="subItem in item.menus" :key="subItem.id" class="item-li" :rel="subItem.id" @touchmove.prevent>
              <p class="p-word">
  ...
  methods: {
    ...
    gtouchend() {
      ...
      if (menuItem && !state) {
        ...
        const newData = this.menuData.map((item) => {
          let obj = {...item};
          if (obj.index === index1) {
            obj.id = this.idObj.id;
            obj.name = this.idObj.name.trim();
            obj = MENUS_TABLE[obj.id];
          }
          return obj;
        });
  ...
  mounted() {
    ...
    document.addEventListener('touchmove', (evt) => {
      ...
      if (newDiv) {
        ...
        // console.log('=======================start');
        // console.log(left);
        // console.log(this.menuPosition.tab1.x);
        // console.log(this.menuPosition.tab2.x);
        // console.log(top);
        // console.log(this.menuPosition.tab1.y);
        // console.log('=======================end');
