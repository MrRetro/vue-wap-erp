<template>
  <cube-popup :visible="visible">
    <div class="pop-content">
      ...
      <yi-list style="height:400px;" :data="list" :enabled-pull-up="false" :enabled-pull-down="false">
      ...
      <div class="bottom">
        ...
        <div class="line"></div>
        <yi-touch class="button sure" @click="handleSureClick">确定{{list.filter(item=>item.check).length}}
        </yi-touch>
      </div>
    </div>
  </cube-popup>
...
export default {
  ...
  methods: {
    handleSureClick() {
      this.onSuccess && this.onSuccess(JSON.parse(JSON.stringify(this.selectList.filter(item => item.check))));
      this.searchText = '';
      this.hide();
    },
    handleCancelClick() {
    ...
    handleItemClick(index) {
      if (this.multiple) {
        if (this.selectList.filter(item => item.check).length >= this.max) {
          return;
        }
        this.selectList[index].check = !this.selectList[index].check;
      } else {
        this.selectList.forEach((item, i) => item.check = i === index);
      }
    },
    /**
       * 显示选人弹窗
