<template>
  <div @click="hide" class="color-box innerBox" :class="{'show': visible, 'hide': !visible}" v-show="visible2">
    <div @click.stop class="content">
      <p class="p-btns">
        <span class="left" @click="cancel">取消</span>
        <span class="right" @click="handleSureClick">确认</span>
      </p>
      <div class="list-box">
        ...
        >
          <ul class="ul-box">
            <yi-touch
              component-name="li"
              :key="index" class="li"
              :class="{'over':selectIndex===index}"
              v-for="(item, index) in colorList"
              @click="selectColor(item,index)"
            >
                <p class="p-box"><yi-inner-color :colors.sync="item"/>
                  <span class="name">
                    {{item.colors[0].name}}<span v-if="item.colors[1]">/{{item.colors[1].name}}</span>
                  </span>
                </p>
            </yi-touch>
          </ul>
...
export default {
  name: 'ChooseInnerColor',
  mounted() {
    this.$AlertUtil.saveInstance('chooseInnerColor', this);
  },
  beforeDestroy() {
    this.$AlertUtil.removeInstance('chooseInnerColor');
  },
  watch: {
    visible(vl) {
      if (!vl) {
  ...
  methods: {
    ...
    show({selKey, colorList, onSuccess}) {
      if (!colorList || colorList === []) {
        ...
        let fn = setTimeout(() => {
          let listBox = document.querySelectorAll('.innerBox .list-box');
          let box = document.querySelectorAll('.innerBox .ul-box');
          let div = document.querySelectorAll('.innerBox .ul-box .li');
          let l = document.querySelectorAll('.innerBox .top');
          let r = document.querySelectorAll('.innerBox .line.bottom');
          let h = div[0].clientHeight;
