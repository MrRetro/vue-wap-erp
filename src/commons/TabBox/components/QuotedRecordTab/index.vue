<template>
  <div>
    ...
    <div v-if="isEditEnabled" style="display: flex;width: 100%;">
      <yi-btn-box v-if="listData.length !== 0 && edit" class="btn cancel" name="取消报价"
                  @onBtnClick="handleCancelClick"/>
      <yi-btn-box v-if="listData.length !== 0" :disabled="btnDisabled" :class="{btn:true,'btn-disabled':btnDisabled}"
                  name="采纳报价"
...
export default {
  ...
  computed: {
    ...
    isEditEnabled() {
      return this.listData.findIndex(item=> item.status.code === 0) !== -1;
    },
    listData() {
      let data = [];
      if (this.type === 'tab') {
      ...
      return data.map(item => ({
        ...
        data: item.quotedPriceMsgList.map(t => {
          const {
            resourceLocation, insuranceType, trafficPrice, deliveryTime, invoiceType, procedureTracks, productionTime,
            quotedPrice, invoicePrice, resourceArea
          } = t;
          return [
  ...
  methods: {
    async fetchData() {
      try {
        const res = await getQuotedRecords(this.id);
        this.data = res;
      } catch (e) {
        this.data = [];
      }
