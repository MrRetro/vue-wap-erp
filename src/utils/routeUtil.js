...
export class RouteUtil {
   ...
   */
  static searchList({module = 'customers', type = 'all', onSuccess, choose = false, needBack = true}) {
    RouteUtil.push({name: 'searchList', params: {type, module, onSuccess, choose, needBack}});
  }
   ...
   * 订单新建 步骤二
   */
  static orderAddThree = ({data1, ...params}, method = 'push') => {
    RouteUtil[method]({name: 'orderAddThree', params: {data1, ...params}});
  };
   ...
   *  取消订单
   */
  static orderCancel = ({id}) => {
    RouteUtil.push({name: 'orderCancel', params: {id}});
  };
   ...
   * 订单合同编辑
   */
  static orderEdit({id, onSuccess}) {
    RouteUtil.push({name: 'orderEdit', params: {id, onSuccess}});
  }
